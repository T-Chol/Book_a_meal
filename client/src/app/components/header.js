    ...
    return (
        <div className="border-b-2 shadow-inner  hover:shadow-teal-600 h-20 flex justify-between items-center">
            ...
            <ul className="flex space-x-6 ml-auto">
                ...
                <Link href="/specials" className={isActive("/specials")}>Today's Special</Link>
                <Link href="/myCart" className={isActive("/myCart")}>Cart 🛒</Link>
            </ul>

            {/* Auth Links */}
            <ul className="ms-auto flex space-x-4 mr-4">
                <Link href="/signup" className={isActiveAuth("/signup")}>Sign Up</Link>
                <Link href="/login" className={isActiveAuth("/login")}>Log in</Link>
            </ul>
        </div>
    );
}
